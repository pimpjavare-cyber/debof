...
_SIGNATURES: dict[ObfuscatorType, list[str | re.Pattern]] = {
    ObfuscatorType.LURAPH14: [
        ...
        re.compile(r'Luraph Obfuscator v14', re.IGNORECASE),
        re.compile(r'xpcall\s*\(.*?function\s*\(\).*?end\s*,'),   # xpcall wrapper
        re.compile(r'local\s+\w+\s*=\s*["\'][A-Za-z0-9+/=]{40,}["\']'),  # b64 payload
        re.compile(r'__LPH_MACRO'),
        re.compile(r'\[==\[LPH\]'),
        "LURAPH_UNIQUE_IDENTIFIER",
        "Luraph decompression error",
    ],
    ObfuscatorType.LUAPROTECT: [
        re.compile(r'LuauProtect|luauprotect', re.IGNORECASE),
        re.compile(r'\[\[LuauProtect v', re.IGNORECASE),
        re.compile(r'luauprotect\.up\.railway\.app'),
        re.compile(r'static_content_\d+'),
        re.compile(r'init-[a-f0-9]+'),
        re.compile(r'game:HttpGet\s*\(\s*["\']https?://luauprotect', re.IGNORECASE),
...
# ── Luauprotect processor ─────────────────────────────────────────────────────

class LuauProtectProcessor(BaseProcessor):
    """
    Luauprotect typically wraps a Luraph 14.x payload behind a small loader that:
      - Creates a static_content_* folder
      - HttpGets the real payload from luauprotect.up.railway.app
      - Caches it to a local init-*.lua file
      - loadstring()s the result

    This processor strips the outer loader and then runs the Luraph 14.x processor
    on whatever remains (or on the original source if no clear loader is found).
    """

    obf_type = ObfuscatorType.LUAPROTECT

    def process(self, source: str) -> str:
        out = source

        # Strip common Luauprotect loader wrappers
        out = re.sub(
            r"--\s*Do not save this file\s*\n?",
            "-- [deobf: Luauprotect header removed]\n",
            out,
            flags=re.IGNORECASE,
        )
        out = re.sub(
            r"--\s*This File Was Generated By\s*\[\[LuauProtect[^\]]*\]\]\s*\n?",
            "-- [deobf: LuauProtect generator header removed]\n",
            out,
            flags=re.IGNORECASE,
        )

        # Remove the typical HttpGet + writefile + loadstring loader block
        loader_patterns = [
            r"makefolder\s*\(\s*['\"]static_content_\d+['\"]\s*\)\s*",
            r"game:HttpGet\s*\(\s*['\"]https?://luauprotect[^'\"]+['\"][^)]*\)",
            r"writefile\s*\(\s*['\"]static_content_[^'\"]+['\"][^)]*\)",
            r"pcall\s*\(\s*makefolder\s*,",
            r"listfiles\s*\(\s*['\"]\./?static_content_",
        ]
        for pat in loader_patterns:
            out = re.sub(pat, "-- [deobf: loader line removed] ", out, flags=re.IGNORECASE)

        # If the source still looks like heavy Luraph, run the Luraph 14 processor
        luraph = Luraph14Processor()
        try:
            out = luraph.process(out)
        except Exception:
            pass
        ...
        if out.strip() == source.strip():
            # Fallback note
            out = (
                "-- [deobf: Luauprotect loader patterns stripped where possible]\n"
                "-- The inner payload appears to be Luraph 14.x. "
                "Try forcing obfuscator = luraph14 for deeper processing.\n\n"
                + out
            )
...
_PROCESSORS: dict[ObfuscatorType, BaseProcessor] = {
    ...
    ObfuscatorType.LUAPROTECT:    LuauProtectProcessor(),
